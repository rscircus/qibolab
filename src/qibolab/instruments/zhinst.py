...
def measure_signal_name(
    qubit: Union[Qubit, int], channel_name: Optional[str] = None
) -> str:
    ...
    """
    if isinstance(qubit, int) and channel_name is None:
        raise ValueError(
            "When specifying qubit by its ID only, readout channel name should be specified as well"
        )
    channel_name = qubit.readout.name if isinstance(qubit, Qubit) else channel_name
    return f"{channel_name}_{qubit.name if isinstance(qubit, Qubit) else qubit}"
    ...
    def sequence_zh(self, sequence, qubits, couplers):
        ...
        for pulse in sequence:
            if pulse.type == PulseType.READOUT:
                zhsequence[measure_signal_name(pulse.qubit, pulse.channel)].append(
                    ZhPulse(pulse)
                )
            else:
        ...
        for sweeper in self.sweepers.copy():
            if sweeper.parameter.name in SWEEPER_SET:
                for pulse in sweeper.pulses:
                    if pulse.type == PulseType.READOUT:
                        aux_list = zhsequence[
                            measure_signal_name(pulse.qubit, pulse.channel)
                        ]
                    else:
    ...
    def sweep_recursion(self, qubits, couplers, exp, exp_calib, exp_options):
        ...
        if sweeper.parameter is Parameter.frequency:
            for pulse in sweeper.pulses:
                ...
                if line == "readout":
                    channel_name = measure_signal_name(pulse.qubit, pulse.channel)
                else:
